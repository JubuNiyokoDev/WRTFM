...
import { submitProofForm, useGetTask, useClaimTask, getGetTaskQueryKey, useListAssignments } from '@/api-client';
...
export default function WorkerTaskDetail() {
  ...
  const submitCorrectionMutation = useMutation({ 
    mutationFn: submitProofForm,
    onSuccess: () => {
      toast({ title: t('worker.task.correction_submitted'), description: t('worker.task.correction_submitted_desc') });
  ...
  const handleClaim = () => {
    claimMutation.mutate({ data: { taskId } }, {
      onSuccess: (assignment) => {
        ...
        setHasClaimed(true);
        toast({ title: t('worker.task.claimed_title'), description: t('worker.task.claimed_desc') });
      },
      onError: (err: any) => {
        const errorMessage = err?.data?.error || err?.message || 'An error occurred';
        toast({
    ...
    const onSubmitProof = (values: z.infer<typeof proofSchema>) => {
      ...
      if (hasCorrectionRequested) {
        submitCorrectionMutation.mutate({
          assignmentId,
          proofType: screenshot && values.link ? 'combined' : screenshot ? 'screenshot' : 'link',
          screenshot,
        ...
        submitMutation.mutate({
          ...
          onSuccess: (res) => {
            ...
            if (res.status === 'manual_review') {
              const assignment = assignments?.items?.find((a: any) => a.id === assignmentId);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-card p-3.5 sm:p-4 rounded-[16px] border border-border shadow-sm flex flex-col md:flex-row justify-between items-start gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            {isUnavailable && <span className="px-2 py-0.5 rounded bg-chart-4/10 text-chart-4 text-xs font-bold uppercase tracking-wider ml-2">{t('worker.task.unavailable')}</span>}
          </div>
        ...
        <div className="flex w-full shrink-0 items-center gap-3 rounded-lg border border-primary/20 bg-primary/10 p-3 text-primary sm:w-auto sm:gap-4">
          <div>
            <div className="text-xs uppercase tracking-wider font-medium opacity-80">{t('general.reward')}</div>
            <div className="metric-value">${task.reward.toFixed(2)}</div>
          </div>
          {task.estimatedMinutes && (
            <div className="pl-4 border-l border-primary/20">
              <div className="text-xs uppercase tracking-wider font-medium opacity-80">{t('worker.task.est_time')}</div>
              <div className="flex items-center gap-1 font-mono text-base font-medium"><Clock className="h-4 w-4" /> {task.estimatedMinutes}m</div>
            </div>
          )}
        </div>
      </div>
      ...
      {submittedResult ? (
        <Card className="border-primary/50 bg-primary/5">
          <CardContent className="space-y-4 p-3.5 text-center sm:p-4">
            <div className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 text-primary sm:h-11 sm:w-11">
              {submittedResult.status === 'auto_approved' ? <CheckCircle2 className="h-6 w-6" /> : <Clock className="h-6 w-6" />}
            </div>
            <AppIllustration kind="proof" className="mx-auto max-w-[180px]" fit="contain" />
            <h2 className="text-base font-display font-bold sm:text-lg">{t('worker.task.proof_submitted')}</h2>
            ...
            <div className="pt-4">
              <Button onClick={() => setLocation('/worker/tasks')}>{t('worker.task.more_tasks')}</Button>
            </div>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            ...
            <CardContent className="space-y-6">
              ...
              {task.targetUrl && (
                <div className="p-4 bg-muted rounded-lg">
                  <p className="text-sm font-medium mb-2">{t('worker.task.target_link')}:</p>
              ...
              {task.proofRequirements && task.proofRequirements.length > 0 && (
                <div>
                  <p className="text-sm font-medium mb-2">{t('worker.task.required_proof')}:</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{hasClaimed ? t('worker.task.submit_proof') : t('worker.task.action_required')}</CardTitle>
              <CardDescription>
                {hasClaimed ? t('worker.task.provide_evidence') : t('worker.task.claim_first')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {!hasClaimed ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <AppIllustration kind="campaign" className="mb-2 max-w-[180px]" fit="contain" />
                  <h3 className="text-lg font-medium mb-2">{t('worker.task.ready')}</h3>
                  <p className="text-sm text-muted-foreground mb-6 max-w-xs">{t('worker.task.reserve_desc')}</p>
                  <Button className="w-full sm:w-auto" onClick={handleClaim} disabled={claimMutation.isPending || isUnavailable}>
                    {claimMutation.isPending ? t('worker.task.claiming') : isUnavailable ? t('worker.task.unavailable') : t('worker.task.claim_task')}
                  </Button>
                </div>
              ) : (
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmitProof)} className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>{t('worker.task.username_desc')}</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>{t('worker.task.public_link_desc')}</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea placeholder={t('worker.task.notes_placeholder')} className="resize-none" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <Button type="submit" className="w-full mt-4" disabled={submitMutation.isPending}>
                      ...
                      {submitMutation.isPending ? t('worker.task.verifying') : t('worker.task.submit_engine')}
                    </Button>
                  </form>
                </Form>
              )}
