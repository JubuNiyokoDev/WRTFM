...
export function AppLayout({ children }: { children: React.ReactNode }) {
      ...
      <div className="min-h-screen bg-background text-foreground flex flex-col font-sans">
        <header className="sticky top-0 z-50 flex h-14 flex-none items-center justify-between border-b border-border/70 bg-background/95 px-5 backdrop-blur-xl sm:h-16 sm:px-6 lg:px-8">
          ...
          >
            ...
            <ProductMark
              showName
              className="hidden max-w-[260px] sm:flex lg:max-w-none"
            />
          </Link>
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            <div
              className="grid grid-cols-2 rounded-md border border-border bg-card p-0.5 shadow-inner"
              aria-label="Language selector"
            >
              <button
                className={cn(
                  "h-8 rounded-[10px] px-2.5 text-xs font-semibold transition-all",
                  lang === "fr"
            ...
            {role ? (
              <Link
                href={`/${role}`}
                className="inline-flex h-9 items-center rounded-md border border-border bg-card px-3 text-xs font-semibold transition-colors hover:text-primary sm:text-sm"
              >
                {t("nav.dashboard")}
              </Link>
            ) : (
              <>
                ...
                <Link
                  href="/auth/register"
                  className="app-button inline-flex h-9 items-center justify-center rounded-md bg-primary px-3 text-xs font-semibold text-primary-foreground shadow-sm transition-all hover:-translate-y-0.5 hover:bg-primary/90 sm:px-3.5 sm:text-sm"
                >
                  {t("nav.register")}
                </Link>
              </>
  ...
  const navItems = {
    ...
    worker: [
      ...
      { href: "/worker/history", label: t("nav.history"), icon: History },
    ...
    <div className="min-h-screen bg-background text-foreground flex font-sans">
      ...
      <aside className="hidden w-64 border-r border-border/70 bg-background/95 backdrop-blur-xl lg:flex flex-col flex-none sticky top-0 h-screen">
        <div className="h-16 flex items-center px-4 border-b border-border/70">
          <Link
            href="/"
            className="font-display font-bold text-lg tracking-tight text-foreground flex items-center gap-2 min-w-0"
          >
        ...
        <div className="p-3 border-t border-border/70 mt-auto">
          ...
          <div className="flex bg-card rounded-[16px] border border-border p-1 mt-4 shadow-inner">
            <button
              className={cn(
                "flex-1 py-1.5 text-xs font-semibold rounded-sm transition-all",
                lang === "fr"
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto pb-20 lg:pb-0">
        <header className="h-16 border-b border-border/70 bg-background/90 backdrop-blur-xl sticky top-0 z-40 flex items-center justify-between px-4 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-11 w-11 flex-none lg:hidden"
            ...
            <Link
              href="/"
              className="flex flex-none items-center lg:hidden"
        ...
        >
          <div className="w-full app-fade-in">{children}</div>
        </motion.div>
      </main>
      <AppBottomNav />
    </div>
