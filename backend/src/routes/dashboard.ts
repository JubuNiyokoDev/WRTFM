...
import { eq, desc, count, sql, inArray, or, and } from "drizzle-orm";
import {
  db,
  usersTable,
  campaignsTable,
...
async function getVisibleTaskIds(user: any): Promise<number[]> {
  if (user.role === "worker") {
    const assignments = await db
      .select({ taskId: assignmentsTable.taskId })
...
router.get("/dashboard/client-summary", async (req, res): Promise<void> => {
  ...
  const tasks = await db
    ...
    .from(tasksTable)
    .where(
      sql`${tasksTable.campaignId} IN (${campaigns
        .map((c) => c.id)
        .concat([0])
        .join(",")})`,
    );
  ...
  const assignments = await db
    ...
    .from(assignmentsTable)
    .where(
      sql`${assignmentsTable.taskId} IN (${tasks
        .map((t) => t.id)
        .concat([0])
        .join(",")})`,
    );
  ...
  const verifications = await db
    ...
    .where(
      sql`${verificationsTable.assignmentId} IN (${assignments
        .map((a) => a.id)
        .concat([0])
        .join(",")})`,
    );

  const totalTasksCompleted = assignments.filter(
    (a) => a.status === "approved",
  ).length;
  const totalSpent = campaigns.reduce((s, c) => s + c.spent, 0);

  const autoDecided = verifications.filter(
    (v) => v.method === "automatic",
  ).length;
  const automationRate =
    verifications.length > 0 ? autoDecided / verifications.length : 0;
  ...
  const topCampaign = campaigns.sort(
    (a, b) => b.workersCompleted - a.workersCompleted,
  )[0];

  res.json(
    GetClientSummaryResponse.parse({
      activeCampaigns,
...
router.get("/dashboard/worker-summary", async (req, res): Promise<void> => {
  ...
  const validationRate =
    assignments.length > 0 ? tasksCompleted / assignments.length : 0;
  ...
  const availableTasks =
    activeCampaignIds.length === 0
      ...
      : await db
          ...
          .where(
            and(
              eq(tasksTable.status, "available"),
              inArray(tasksTable.campaignId, activeCampaignIds),
            ),
          )
  ...
  res.json(
    GetWorkerSummaryResponse.parse({
      tasksCompleted,
...
router.get("/dashboard/admin-summary", async (req, res): Promise<void> => {
  ...
  const [
    ...
    db
      .select({ count: count() })
      .from(verificationsTable)
      .where(eq(verificationsTable.status, "manual_review")),
  ]);
  ...
  const wallets = await db
    .select({ totalEarned: walletsTable.totalEarned })
    .from(walletsTable);
...
router.get("/dashboard/activity-feed", async (req, res): Promise<void> => {
  ...
  if (currentUser.role === "worker")
    visibilityConditions.push(eq(activityEventsTable.userId, currentUser.id));
  if (taskIds.length > 0)
    visibilityConditions.push(inArray(activityEventsTable.taskId, taskIds));
  if (campaignIds.length > 0)
    visibilityConditions.push(
      inArray(activityEventsTable.campaignId, campaignIds),
    );
  ...
  const events = await db
    ...
    .where(
      currentUser.role === "admin" ? undefined : or(...visibilityConditions),
    )
...
router.get("/dashboard/automation-stats", async (req, res): Promise<void> => {
  ...
  const autoApproved = verifications.filter(
    (v) => v.status === "auto_approved",
  ).length;
  const autoRejected = verifications.filter(
    (v) => v.status === "auto_rejected",
  ).length;
  ...
  const byTaskType = Object.entries(byType).map(([taskType, stats]) => ({
    taskType,
    automationRate:
      stats.total > 0 ? Math.round((stats.auto / stats.total) * 100) / 100 : 0,
    count: stats.total,
  }));
